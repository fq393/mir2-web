...
- 原注册/登录/改密/创建窗口与真实账户；战法道、男女性别、角色列表及退出重登；自动选中、原生双人物位和重名中文反馈。
...
- 包裹与人物窗独立并开，包裹物品单击携带/交换、点击装备位、双击使用；原登录错误消息框，见[窗口与鼠标流程](docs/native-window-flow.md)。三类服务端负重仍保留，人物属性/负重分页继续完善。
...
已接原石门背景、登录成功后的开门动画、登录/选角原音乐，以及男女战法道人物预览。首次点击后启用音乐，可在游戏画布外的网页控件中静音；进入地图后停止登录音乐。取证、构建与验收见[登录流程](docs/login-presentation.md)。
