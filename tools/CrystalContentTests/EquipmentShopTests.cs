...
static class EquipmentShopTests {
 public static void Run(string root,Envir envir,MirConnection connection){
  ...
  foreach(var spec in new[]{("BoundarySmith",9),("BoundaryTailor",6)}){
  ...
  Console.WriteLine("PASS 15 sourced equipment purchases: original stock, shape/image/durability/gender/level, exact charge, full-bag protection and idempotent NPCs.");
 }
}
