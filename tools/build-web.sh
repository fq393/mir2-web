...
if ! cmp -s "$project_root/server/content/world-maps.json" "$project_root/client/assets/resources/mir/world-maps.json"; then echo 'Bundled map registry does not match server/content/world-maps.json'; exit 1; fi
for asset in "$project_root/client/assets/resources/mir/webui/frames.json" "$project_root/client/assets/resources/mir/actors/healing.json" "$project_root/assets/auth-web/manifest.json"; do
  if [ ! -f "$asset" ]; then echo "Bundled game asset is missing: $asset"; exit 1; fi
done
