...
crystal_healthy() {
  curl --max-time 2 -fsS http://127.0.0.1:17080/health | python3 -c 'import json,sys; d=json.load(sys.stdin);sys.exit(not(d.get("engine")=="Suprcode/Crystal" and d.get("running")))' 2>/dev/null
}
...
if curl --max-time 2 -fsS http://127.0.0.1:17600/__mir2_health | python3 -c 'import json,sys;d=json.load(sys.stdin);sys.exit(not(d.get("project")=="mir2-web" and d.get("root")==sys.argv[1] and d.get("webBuild")))' "$project_root" 2>/dev/null;then
