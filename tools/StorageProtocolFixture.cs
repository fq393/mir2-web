...
static class StorageProtocolFixture
{
    public static void Seed(Envir envir, string root)
    {
        ...
        bool native=File.Exists(Path.Combine(root,"WAREHOUSE_UI_QA"));
        var npc=envir.NPCInfoList.Single(n=>n.FileName==(native?"BoundaryWarehouse":"BichonJewellery4"));
        if(!native)File.WriteAllText(Path.Combine(Settings.NPCPath,npc.FileName+".txt"),"[@MAIN]\n#SAY\n仓库协议测试\n<保管/@STORAGE>\n\n[@STORAGE]\n#SAY\n请选择物品。\n");
        envir.SaveDB();
        ...
        foreach(var name in new[]{"仓库协议甲","仓库协议乙"})
        {
            var role=new CharacterInfo{Index=++envir.NextCharacterID,Name=name,AccountInfo=account,Class=MirClass.Warrior,Gender=MirGender.Male,Level=1,HP=18,MP=14,
                CurrentMapIndex=native?envir.MapInfoList.Single(m=>m.FileName=="0").Index:npc.MapIndex,CurrentLocation=native?new Point(306,627):new Point(npc.Location.X+1,npc.Location.Y),BindMapIndex=npc.MapIndex,BindLocation=new Point(npc.Location.X+1,npc.Location.Y),CreationIP="127.0.0.1",CreationDate=DateTime.Now};
            ...
            if(!native)CharacterStorage.Initialize(role,2);account.Characters.Add(role);envir.CharacterList.Add(role);
        }
