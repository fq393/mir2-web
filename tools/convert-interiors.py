...
OUT=ROOT/'client/assets/resources/mir/maps/0105'
 ...
 for y in range(h):
  for x in range(w):
   ...
   for layer,library,index in [('back',0,(back&32767)-1),('middle',1,middle-1),('front',lib+2,(front&32767)-1)]:
    if index<0:continue
    ref=dict(library=library,index=index,key=f'room0105:{library}:{index}',render=layer!='back' or (x%2==0 and y%2==0))
    if layer=='front' and anim&127:ref['animationKeys']=[f'room0105:{library}:{index+i}' for i in range(anim&127)]
    c[layer]=ref
   cells.append(c)
 return w,h,cells
...
def build():
 ...
 pin=json.loads((ROOT/'tools/interior-inputs.json').read_text())
 ...
 w,h,cells=read_map(SOURCE/'Map/0105.map');wanted={}
 for c in cells:
 ...
 for lib,indices in wanted.items():
  ...
  images.extend((f'room0105:{lib}:{i}',im,meta) for i,(im,meta) in result.items())
 atlas=Image.new('RGBA',(2048,2048));frames={};atlases=[];x=y=row=0
 def flush():
  nonlocal atlas,x,y,row
  height=1<<(max(1,y+row)-1).bit_length();file=f'maps/0105/atlas-{len(atlases)}.png'
 ...
 source=SOURCE/'Map/0105.map';sources.append(dict(path=str(source.relative_to(ROOT)),sha256=hashlib.sha256(source.read_bytes()).hexdigest()))
 data=dict(map=dict(id='0105',name='首饰店',width=w,height=h,originX=0,originY=0,spawn=dict(x=8,y=24),collisionFile='maps/0105/collision.json',chunks=[dict(x=0,y=0,width=w,height=h,file='maps/0105/cells.json',atlases=list(range(len(atlases))))]),frames=frames,atlases=atlases,sources=sources,portals=PORTALS,portalSource='https://github.com/mrzhqiang/mirserver-1.76/blob/39e17246a32247a2c43c4cc481e97e88c692fa53/Mir200/Envir/MapInfo.txt#L85-L92')
 (OUT/'manifest.json').write_text(json.dumps(data,ensure_ascii=False,separators=(',',':'))+'\n')
 print(json.dumps(dict(map='0105',frames=len(frames),atlases=len(atlases),cells=len(cells))))
