...
import argparse,json,shutil,uuid,xml.etree.ElementTree as ET
parser=argparse.ArgumentParser();parser.add_argument("--bookshop",action="store_true");args=parser.parse_args()
qa_name="bookshop-qa" if args.bookshop else "skillkeys-qa"
fixture="BookshopFixture" if args.bookshop else "SkillKeysFixture"
root=Path(__file__).resolve().parents[1]
qa=root/'.runtime'/qa_name
if qa.exists():raise SystemExit('QA root already exists; keep it for relogin checks. Refusing to overwrite.')
qa.mkdir(mode=0o700)
(qa/('BOOKSHOP_QA_ONLY' if args.bookshop else 'SKILL_KEYS_QA_ONLY')).write_text('Isolated synthetic skills; not 1.76 content.\n')
...
program=program.replace('const int crystalPort = 17000;','const int crystalPort = 17100;').replace('127.0.0.1:17080','127.0.0.1:17180').replace('127.0.0.1:17000','127.0.0.1:17100').replace('envir.Start();',fixture+'.Seed(envir,repoRoot);\nenvir.Start();')
...
ET.SubElement(project.find('.//ItemGroup'),'Compile',Include=str(root/'tools/fixtures'/(fixture+'.cs')))
...
print(f'Host: MIR2_ROOT="$PWD/.runtime/{qa_name}" .runtime/dotnet/dotnet run --project .runtime/{qa_name}/server/SkillKeysQA.csproj')
print(f'Web: python3 -m http.server 17610 --bind 127.0.0.1 --directory .runtime/{qa_name}/web')
