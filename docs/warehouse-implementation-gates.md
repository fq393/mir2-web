...
- 网页协议与完整状态已接入（2026-09-13，见storage-protocol.md），原版UI尚未接入。NPC出现不代表存取功能完成。
...
- [x] 0140/NPC9和存取主流程已接入，浏览器验收见warehouse-native-ui.md；拖动操作等细节仍未完成。

- [x] 后端存取事务及真实保存/加载、失败返还测试，见storage-transfers.md。
- [x] 网页会话保护、NPC/地图及原框体存取主流程已验收；剩余项见warehouse-native-ui.md。
