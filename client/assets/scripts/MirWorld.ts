...
export class MirWorld extends Component {
    ...
    private lastFrame='';
    private ready=false;private hudRows:string[]=[];private panelRects:PanelRect[]=[];private inventoryPage='bag';private shopTop=0;private shopBagOpen=true;private hovered=0;
    ...
    private notice(text:string):void {this.logs.push(text);this.logs=this.logs.slice(-4);if(this.logText)this.logText.string=this.logs.join('\n');}
    private attack():void {
    ...
    private cast():void {
        ...
        if(!this.serverReady||this.step||this.hp<=0||this.actionTime>0)return;const p=this.peers.get(this.selected);
        if(!p||p.kind!=='monster'||p.dead){this.notice('先选择怪物，再按 F1 施放火球');return;}
    ...
    private closeNative(parent:Node,x:number,y:number,action:()=>void=()=>{this.menu.active=false;}):void {const button=this.nativeCrop(parent,'ui:ClassicPrguse:370',x,y,8,40,15,23);this.nativeClick(button.node,action);}
    ...
    private usePotion(item:any):void {
        ...
        if(info?.type!==13){this.notice('此格物品不能作为药剂服用');return;}
    ...
    private showInventory(page=this.inventoryPage):void {
        ...
        if(page==='bag'){this.renderBag(0);return;}const character=this.nativeWindow(this.menu,'ui:ClassicPrguse:370',568,0);
        this.nativeImage(character,'ui:ClassicPrguse:376',40,53);this.nativeLabel(character,'装备',96,66,12,92,Color.WHITE);this.nativeLabel(character,'旅人',84,24,13,145,C.gold);this.closeNative(character,8,40);
        ...
        const remove=(item:any)=>{const to=this.inventory.findIndex((v,i)=>!v&&i>=6);if(to>=0)this.connection?.send({type:'unequip',uniqueId:String(item.uniqueid),to});else this.notice('背包空间不足。');};
        for(const {slot,x,y} of JEWELLERY_SLOTS){const item=this.equipment[slot];if(item)this.nativeItem(character,item,x,y,()=>remove(item));}
    ...
    private renderBag(x:number):void {
        ...
        for(let slot=6;slot<Math.min(46,this.inventory.length);slot++){
            ...
            this.nativeItem(bag,item,x,y,()=>{const info=item.info??this.itemInfo.get(item.itemindex);const target=equipmentTarget(info?.type,this.equipment);if(info?.type===13)this.usePotion(item);else if(target>=0)this.connection?.send({type:'equip',uniqueId:String(item.uniqueid),slot:target});});
        }
    }
    private nativeItem(parent:Node,item:any,x:number,y:number,action:()=>void):void {
        const info=item.info??this.itemInfo.get(item.itemindex),key=`ui:Items:${info?.image}`;if(!this.frames.has(key))return;
        const f=this.frames.get(key)!;const icon=this.nativeImage(parent,key,x+(36-f.meta.w)/2,y+(32-f.meta.h)/2);this.nativeClick(icon.node,action);
        icon.node.on(Node.EventType.MOUSE_ENTER,()=>this.targetText.string=`${this.itemName(item)}${info?.type===13?' · 持续恢复 '+((info.hp??info.stats?.values?.hp)?'HP '+(info.hp??info.stats.values.hp):'MP '+(info.mp??info.stats?.values?.mp??0)):''} · ${info?.price??25} 金币`);
    ...
    private showShop():void {
        ...
        for(const [text,x] of [['物品列表',19],['费用',156],['持久',245]] as const)this.nativeLabel(dialog,text,x,17,12,130,Color.WHITE);
    ...
    private packet(name:string,raw:any):void {
        ...
        if(name==='UseItem'){
            ...
            if(consumeItem(this.inventory,key,d.success)){this.sound.play('108');this.notice(`服用 ${this.itemName(item)}`);this.refreshBelt();if(this.menu.active){if(this.menuKind==='inventory')this.showInventory();else if(this.menuKind==='shop')this.showShop();}}
            else if(!d.success)this.notice('药剂未使用：服务器拒绝此次操作');
        }
        ...
        if(name==='Chat'&&d.message)this.notice(d.message);

    }
    onDestroy():void {this.resizeObserver?.disconnect();this.sound.destroy();
