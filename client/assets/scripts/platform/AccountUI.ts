...
export class AccountUI {
 ...
 event(e:any):void{if(e.type==='error'){this.message.textContent=e.message??'操作未成功';if(!this.waitingForConnection)this.canvas.querySelectorAll('button').forEach(b=>b.disabled=false);return;}if(e.type==='ready'){this.exit.hidden=false;this.root.hidden=true;this.canvas.replaceChildren();document.getElementById("GameCanvas")?.focus();return;}if(e.type==='disconnected'){this.characters=[];this.selected=0;this.waitingForConnection=true;this.show('login','正在重新连接服务器…');this.lockConnectionForm();return;}if(e.type==='auth'){this.waitingForConnection=false;this.characters=e.characters??this.characters;if(Number.isInteger(e.characterLimit)&&e.characterLimit>0)this.characterLimit=e.characterLimit;if(e.selectedIndex)this.selected=e.selectedIndex;this.selected=characterListState(this.characters,this.selected).selected;this.show(e.stage,e.message??'');}}
 ...
 show(stage:string,message=''):void{
  ...
  if(stage==='login'){
   ...
   state.characters.slice(state.page*2,state.page*2+2).forEach((c,i)=>{
    ...
    for(const [value,y] of [[c.name,493],[c.level?String(c.level):'初入玛法',522],[(c.gender?'女':'男')+(['战士','法师','道士'][c.role]??'未支持'),552]] as const){const text=document.createElement('div');text.textContent=value;text.style.cssText=`position:absolute;left:${83+i*548}px;top:${y}px;width:110px;height:20px;color:#e9dba7;font:12px SimSun,"Songti SC",serif;`;p.append(text);}
   });
