import {parseGoldAmount} from '../core/gold';
...
export class NativeConfirm {
 private overlay:HTMLDivElement;private amountInput?:HTMLInputElement;private amountHint?:HTMLDivElement;
 value=0;
 ...
 constructor(parent:HTMLElement,text:string,private choose:(accepted:boolean)=>void,mode:'confirm'|'alert'|'choice'|'amount'='confirm',labels:readonly [string,string,string]=['确认删除','不删除','取消删除'],private maximum=4294967295){
  ...
  if(mode==='amount'){
   this.amountHint=label;this.amountInput=document.createElement('input');this.amountInput.setAttribute('aria-label','丢弃金币数量');this.amountInput.inputMode='numeric';this.amountInput.maxLength=10;
   this.amountInput.style.cssText=`position:absolute;left:35px;top:72px;width:382px;height:20px;background:black;color:white;border:1px solid #716b60;font:12px ${CLASSIC_FONT_FAMILY};`;panel.append(this.amountInput);
  }
  const buttons:HTMLButtonElement[]=[];
  const choices:readonly (readonly [string,number,number,boolean])[]=mode==='alert'?[['确定',361,324,true]]:mode==='amount'?[[labels[0],361,214,true],[labels[1],365,324,false]]:mode==='choice'?[[labels[0],363,214,true],[labels[1],367,324,false]]:[[labels[0],363,104,true],[labels[1],367,214,false],[labels[2],365,324,false]];
  ...
  panel.onkeydown=event=>{
   ...
   if(event.key==='Enter'){event.preventDefault();event.stopPropagation();if(mode==='alert'||mode==='amount')this.finish(true);}
   if(event.key==='Tab'){event.preventDefault();const controls:HTMLElement[]=this.amountInput?[this.amountInput,...buttons]:buttons;const i=controls.indexOf(document.activeElement as HTMLElement);controls[i<0?(event.shiftKey?controls.length-1:0):(i+(event.shiftKey?-1:1)+controls.length)%controls.length].focus();}
  };
  this.trapFocus=event=>{if(!this.finished&&!this.overlay.contains(event.target as Node))panel.focus();};
  document.addEventListener('focusin',this.trapFocus);if(this.amountInput)this.amountInput.focus();else panel.focus();
 }
 private finish(accepted:boolean):void{if(this.finished)return;if(accepted&&this.amountInput){const value=parseGoldAmount(this.amountInput.value,this.maximum);if(value===null){this.amountHint!.textContent=`请输入1至${this.maximum}的整数金额。`;this.amountInput.focus();return;}this.value=value;}this.destroy();this.choose(accepted);}
