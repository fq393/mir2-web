...
export class ChatInput {
 ...
 constructor(private send:(text:string)=>boolean,private pause:()=>void,private enabled:()=>boolean=()=>true){
  ...
  this.history=document.createElement('div');this.history.setAttribute('aria-label','聊天记录');this.history.style.cssText='position:absolute;left:26%;top:82%;width:48.25%;height:14.5%;overflow:auto;pointer-events:auto;white-space:pre-wrap;overflow-wrap:anywhere;font-size:12px;line-height:13px;';this.root.append(this.history);
  this.input=document.createElement('input');this.input.setAttribute('aria-label','聊天输入');this.input.placeholder='';this.input.hidden=true;this.input.maxLength=80;this.input.style.cssText=`position:absolute;left:26%;top:96.833333%;width:48.25%;height:2%;background:#c0c0c0;color:black;border:0;outline:0;padding:0;font:12px ${CLASSIC_FONT_FAMILY};pointer-events:auto;`;this.root.append(this.input);
  const resize=()=>{const scale=host.clientWidth/800;this.history.style.fontSize=`${12*scale}px`;this.history.style.lineHeight=`${13*scale}px`;this.input.style.fontSize=`${12*scale}px`;};
