...
export class ChatInput {
 ...
 constructor(private send:(text:string)=>boolean,private pause:()=>void,private enabled:()=>boolean=()=>true){
  ...
  this.root=document.createElement('div');this.root.style.cssText='position:absolute;inset:0;pointer-events:none;z-index:10;font-family:SimSun,"Songti SC",serif;';host.append(this.root);this.root.hidden=true;
  this.history=document.createElement('div');this.history.setAttribute('aria-label','聊天记录');this.history.style.cssText='position:absolute;left:26.5%;top:79%;width:47.1%;height:13.4%;overflow:auto;pointer-events:auto;white-space:pre-wrap;overflow-wrap:anywhere;font-size:12px;line-height:16px;';this.root.append(this.history);
  this.input=document.createElement('input');this.input.setAttribute('aria-label','聊天输入');this.input.placeholder='回车聊天：普通文字 /名字 私聊 · !喊话 · !!队聊 · !~行会';this.input.maxLength=80;this.input.style.cssText='position:absolute;left:26.5%;top:94.2%;width:47.1%;height:3.1%;background:#100c0a;color:white;border:0;outline:0;padding:0;font:12px SimSun,"Songti SC",serif;pointer-events:auto;';this.root.append(this.input);
 ...
 destroy():void{window.removeEventListener('keydown',this.key,true);this.root.remove();}
}
