...
export class AuthPresentation {
 ...
 private visibility=()=>{if(document.hidden){this.music.pause();this.doorSound.pause();this.melt.pause();}else this.resume();};
 readonly errors:string[]=[];
 private melt=new Audio('webui/auth/melt.wav');
 selectSound():void{if(this.muted)return;this.unlock();this.melt.currentTime=0;void this.melt.play().catch(e=>{if(e.name!=='NotAllowedError'&&e.name!=='AbortError')this.errors.push(String(e));});}
 ...
 toggle():boolean{this.muted=!this.muted;if(this.muted){this.music.pause();this.doorSound.pause();this.melt.pause();}else this.unlock();return !this.muted;}
 ...
 clear():void{this.generation++;this.timers.forEach(clearInterval);this.timers.clear();this.previews.clear();this.doorSound.pause();this.melt.pause();}
 ...
 updatePortrait(node:HTMLElement,role:number,gender:number,slot=0,selected=true,transition=false):void{
  ...
  if(transition){
   const frozen=portraitLayout(role,gender,slot,true);node.style.left=frozen.x+'px';node.style.top=frozen.y+'px';node.style.backgroundImage='none';
   const stone=new Image(),light=new Image();stone.draggable=false;light.draggable=false;node.append(stone);
   light.style.cssText=`position:absolute;left:${90+slot*340-frozen.x}px;top:${(slot?60:58)-frozen.y}px;mix-blend-mode:plus-lighter;pointer-events:none;`;
   if(selected)node.append(light);
   const started=performance.now();const paint=()=>{const frame=Math.min(12,Math.floor((performance.now()-started)/50));stone.src=`webui/auth/freeze-${role}-${gender}-${selected?frame:12-frame}.png`;if(selected)light.src=`webui/auth/select-effect-${frame}.png`;};paint();
   const timer=setInterval(()=>{if(performance.now()-started>=650){this.updatePortrait(node,role,gender,slot,selected);return;}paint();},25);this.timers.add(timer);this.previews.set(node,timer);return;
  }
